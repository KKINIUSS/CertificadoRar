...
app = Flask(__name__)
UPLOAD_FOLDER = 'C:\\Users\\Kisel\\Desktop\\certificado_algorand-master\\certificado_algorand-master\\static\\temp'
...
def create_asset(algod_client, address, mnemonic_key, description, course):
...
def api():
    ...
    if(query):
        ...
        params.flat_fee = True
        params.fee = 1000
        with open('C:\\Users\\Kisel\\Desktop\\certificado_algorand-master\\certificado_algorand-master\\static\\files\\%s.csv' % (str(file_name)), 'w', newline='') as csvfile:
            time_sec = datetime.datetime.now()
            ...
            for i in range(len(student)):
                ...
                threads.append(t)
            for t in threads:
                asset_id_mas.append(t.join())
            for i, asset_id in zip(student, asset_id_mas):
                # asset_id = random.randint(10000000, 99999999)
        ...
        msg = MIMEMultipart()
        filepath = 'C:\\Users\\Kisel\\Desktop\\certificado_algorand-master\\certificado_algorand-master\\static\\files\\%s.csv' % str(file_name)
...
<body>
    ...
    <div style="margin-top: 1%;">
        ...
        time_sec_end = datetime.datetime.now()
        print(time_sec_end - time_sec)
...
def download_example():
    return send_from_directory('C:\\Users\\Kisel\\Desktop\\certificado_algorand-master\\certificado_algorand-master\\certificates', 'example_template.jpg', as_attachment=False)
...
def certificate(name, code, asset):
    ...
    if(temp[0][0]):
        if(temp[0][0] != 'on'):
            if(temp[0][0] == '3'):
                return render_template("index.html", name=name, back= "/static/" + temp[0][0] + ".png", link="https://algoexplorer.io/asset/" + asset, style=' #000000', description= temp[0][2])
            else:
                return render_template("index.html", name=name, back= "/static/" + temp[0][0] + ".png", link="https://algoexplorer.io/asset/" + asset, style=' #ffffff', description= temp[0][2])
        else:
            ...
            return render_template("index.html", name=name, back=url_for('static' , filename='temp/' + file_name), link="https://algoexplorer.io/asset/" + asset, style=' #ffffff', description= temp[0][2])
    else:
        return "Ошибка"
...
def download_certificate(asset_id, name):
    ...
    print(name)
    return send_from_directory("C:\\Users\\Kisel\\Desktop\\certificado_algorand-master\\certificado_algorand-master\\certificates", name + "_" + asset_id + ".pdf", as_attachment=False)

@app.route('/download/<filename>', methods=["GET", "POST"])
def download(filename):
    print(filename)
    return send_from_directory("C:\\Users\\Kisel\\Desktop\\certificado_algorand-master\\certificado_algorand-master\\static\\files", filename, as_attachment=False)
